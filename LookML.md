...
Excellent question! This is a common point of confusion in LookML. Let me explain why these fields are defined as dimensions rather than measures in that example, and when you should use each approach.

## Why These Are Dimensions in the Example

### 1. **`price`** - Static Product Attribute
```lookml
dimension: price {
  type: number
  sql: ${TABLE}.price ;;
...
```
**Why dimension?** Price is an inherent **attribute** of each product that doesn't change based on aggregation. It's a characteristic that describes the product, similar to product name or category. Each product has one price that remains constant regardless of how you slice the data.

### 2. **`total_quantity_sold`** and **`total_revenue`** - Pre-calculated Values
```lookml
dimension: total_quantity_sold {
  type: number
  sql: 
...
**Why dimensions?** These are **pre-aggregated, product-level attributes** calculated via subqueries. They represent the total historical performance for each individual product, not aggregations that change based on the current query's GROUP BY clause.

## The Key Distinction: Row-Level vs Query-Level Aggregation

### Dimensions: Row-Level Values
```lookml
# This returns one value per product row
dimension: total_lifetime_sales {
  type: number
  sql: (SELECT SUM(amount) FROM sales WHERE product_id = ${product_id}) ;;
  # Each product gets its own total - this is a product attribute
}
```

### Measures: Query-Level Aggregations
```lookml
# This aggregates across whatever is in your current result set
measure: total_sales_in_query {
  type: sum
  sql: ${order_amount} ;;
  # Sums up all the order amounts in your current query results
}
```

## Practical Example: The Difference in Action

Let's see how this plays out with actual data:

```lookml
view: product_performance_corrected {
  ...
  dimension: list_price {
    type: number
    sql: ${TABLE}.price ;;
    # This is the product's catalog price - doesn't change based on query
  }
  
  # PRE-CALCULATED LIFETIME METRICS (Dimensions)
  dimension: lifetime_quantity_sold {
    type: number
    sql: 
      (SELECT COALESCE(SUM(quantity), 0) 
       FROM order_items oi 
       WHERE oi.product_id = ${product_id}) ;;
    # Total quantity ever sold for this specific product
  }
  ...
  dimension: lifetime_revenue {
    ...
    sql: 
      (SELECT COALESCE(SUM(quantity * unit_price), 0) 
       FROM order_items oi 
       WHERE oi.product_id = ${product_id}) ;;
    # Total revenue ever generated by this specific product
  }
  
  # QUERY-LEVEL AGGREGATIONS (Measures)
  measure: products_count {
    type: count
    # How many products are in my current query results?
  }
  
  measure: avg_lifetime_revenue {
    type: average
    sql: ${lifetime_revenue} ;;
    # What's the average lifetime revenue of products in my current selection?
  }
  
  measure: total_combined_revenue {
    type: sum
    sql: ${lifetime_revenue} ;;
    # What's the combined lifetime revenue of all products in my current selection?
  }
}
```
...
dimension: product_price {
  sql: ${TABLE}.price ;;  # Each product has one price
}

# 2. Pre-calculated customer lifetime metrics
dimension: customer_lifetime_value {
  sql: ${TABLE}.calculated_clv ;;  # Each customer has one CLV
}

# 3. Derived attributes that describe the entity
dimension: price_tier {
  sql: 
    CASE 
      WHEN ${product_price} > 100 THEN 'Premium'
      ELSE 'Standard' 
    END ;;
}

# 4. Subquery calculations that return one value per row
dimension: days_since_last_order {
  sql: 
    (SELECT DATE_DIFF(CURRENT_DATE, MAX(order_date), DAY) 
     FROM orders 
...
### Use Measures When:
```lookml
# 1. Aggregating transaction-level data
measure: total_sales {
  type: sum
  sql: ${order_amount} ;;  # Sum across filtered orders
}

# 2. Counting occurrences
measure: order_count {
  type: count  # Count orders in current query
}

# 3. Statistical calculations
measure: average_order_value {
  type: average
  sql: ${order_amount} ;;  # Average of filtered orders
}

# 4. Complex aggregations
measure: unique_customers {
  type: count_distinct
  sql: ${customer_id} ;;  # Distinct customers in current results
}
```

## Real-World Comparison

### Scenario: Analyzing Product Performance by Category

**Using Dimensions (Pre-calculated totals):**
```lookml
explore: product_analysis {
  # Query: Show me all Electronics products and their lifetime performance
  # Result: 
  # Product A | Electronics | $50  | 1,000 units | $50,000 lifetime
  # Product B | Electronics | $100 | 500 units   | $50,000 lifetime
  # Product C | Electronics | $25  | 2,000 units | $50,000 lifetime
  
  # The lifetime values are fixed per product
}
```

**Using Measures (Query-driven aggregation):**
```lookml
explore: order_analysis {
  # Query: Show me Electronics sales for Q1 2024
  # Result:
  # Electronics | 500 orders | $75,000 revenue (only Q1 2024 data)
  
  # The measures change based on your date filters
}
```

## The Problem with Making Them Measures

If we incorrectly made `total_quantity_sold` a measure:

```lookml
# INCORRECT APPROACH
measure: total_quantity_sold {
  type: sum
  sql: (SELECT SUM(quantity) FROM order_items WHERE product_id = ${product_id}) ;;
}
```

**Problems:**
1. **Double aggregation:** You're summing subqueries, which could lead to incorrect totals
2. **Performance issues:** Running subqueries inside aggregations is inefficient
3. **Confusing results:** The numbers won't behave as users expect when filtering

## Corrected Approach for Different Use Cases

### If You Want Measures (Transaction-Level Analysis):
```lookml
view: order_items {
  dimension: quantity {
    type: number
...
### If You Want Dimensions (Product-Level Analysis):
```lookml
view: products_with_performance {
  # Keep as dimensions for product-level attributes
  dimension: lifetime_quantity_sold {
    type: number
    sql: ${TABLE}.total_quantity_ever_sold ;;  # Pre-calculated in ETL
  }
  
  dimension: lifetime_revenue {
    type: number  
    sql: ${TABLE}.total_revenue_ever ;;  # Pre-calculated in ETL
  }
}
```
